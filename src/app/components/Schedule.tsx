...
export const Schedule = () => {
  ...
  return (
    <section className="pt-24 pb-8 bg-[#0a0015] text-white px-6 min-h-screen">
      <h2 className="text-center text-5xl md:text-6xl font-bold text-pink-500 mb-8 tracking-[0.1em]" style={{ textShadow: "0 0 10px #ec4899", fontFamily: "'Press Start 2P', cursive" }}>
        SCHEDULE
      </h2>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto mb-16">
        {days.map((day, index) => (
          <button key={index} onClick={() => setActiveTab(index)} className={`p-6 border-2 transition-all duration-300 ${activeTab === index ? "border-pink-500 bg-pink-900/20 shadow-[0_0_20px_rgba(236,72,153,0.4)]" : "border-gray-800 bg-[#131313] hover:border-gray-600"}`}>
          ...
          {days[activeTab].events.map((event, i) => (
            <div key={i} className="flex items-start gap-6">
              ...
              <div className="flex-1 bg-[#131313] p-6 rounded-lg border border-gray-800 shadow-lg">
                <span className={`text-[10px] font-bold uppercase tracking-widest ${categoryColors[event.category]}`}>{event.category}</span>
                <h4 className="text-base font-semibold mt-2">{event.title}</h4>
                <div className="flex items-center gap-2 mt-2 text-xs text-gray-400"><MapPin size={14} /> {event.venue}</div>
              </div>
            </div>
          ))}
        </div>
